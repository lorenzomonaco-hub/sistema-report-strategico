...
/** Invia un'email semplice (senza allegati). */
export async function inviaEmail(opts: { token: string; a: string; oggetto: string; corpo: string }): Promise<{ ok: boolean }> {
  const fd = new FormData()
  fd.append('a', opts.a)
  fd.append('oggetto', opts.oggetto)
  fd.append('corpo', opts.corpo)
  const r = await fetch(`${URL_EMAIL}/invia`, { method: 'POST', headers: { Authorization: `Bearer ${opts.token}` }, body: fd })
  if (!r.ok) {
