...
import { assessFirstCompleti, documentiTutorPronti, indiceFase, statoCommerciale } from '@/lib/fasi'
...
function CartaRaccolta({ pratica, onConfermata }: { pratica: Pratica; onConfermata: (azienda: string) => void }) {
  ...
  return (
    <div className="card-sollevabile rounded-2xl border border-linea bg-carta p-5 shadow-sm">
      ...
      {/* AssessFirst per persona: 4 file a testa */}
      <div className="mt-4">
        <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-inchiostro/40">AssessFirst — 4 file per persona</p>
        <div className="space-y-3">
          {pratica.dipendenti.map((d) => {
            const completa = assessFirstCompleti(pratica, d.nome)
            return (
              <div key={d.nome} className={`rounded-xl border p-3 ${completa ? 'border-green-200 bg-green-50/40' : 'border-linea bg-inchiostro/[0.015]'}`}>
                <div className="mb-2 flex items-center gap-2">
                  <span className="text-sm font-semibold text-inchiostro">{d.nome}</span>
                  <span className="rounded-full bg-inchiostro/5 px-2 py-0.5 text-[11px] text-inchiostro/55">{d.ruolo}</span>
                  {completa && <span className="ml-auto text-xs font-semibold text-green-700">4/4 ✓</span>}
                </div>
                <div className="grid gap-1.5 sm:grid-cols-2">
                  {ASSESSFIRST_TIPI.map((s) => (
                    <SlotUpload key={s} pratica={pratica} categoria="assessfirst" sottotipo={s} dipendente={d.nome} label={s} />
                  ))}
                </div>
              </div>
            )
